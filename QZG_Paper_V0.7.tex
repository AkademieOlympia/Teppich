...
\subsection{Trace-Formel als spektral--arithmetische Rotation \texorpdfstring{$t \leftrightarrow \log p$}{t <-> log p}}
...
Damit folgt aus Proposition~\ref{prop:explicit_QZG_de} unmittelbar die Trace-Identität
\begin{align}
\mathrm{Tr}\,h(D)
...
über die Variablen $u=m\log p$. In diesem präzisen Sinn realisiert \eqref{eq:trace_QZG_de}
...
\]
Dann konvergiert die Primzahlsumme in \eqref{eq:trace_QZG_de} schnell,
